...
def convert_to_eucjp(string)
  NKF.nkf('-exm0', string).bytes.map { |byte| format('$%02X', byte) }.join(', ')
end

def convert_ca65_japanese_to_eucjp(input_filename, output_filename)
    ...
    File.open(output_filename, 'w:utf-8') do |output|
      input.each_line do |line|
        ...
        quoted_parts.each do |match|
          content = match[0]
          if content =~ /[^\x00-\x7F]/
            eucjp_string = convert_to_eucjp(content)
            new_line.sub!("\"#{content}\"", eucjp_string)
          end
        end
...
if options[:input] && options[:output]
  convert_ca65_japanese_to_eucjp(options[:input], options[:output])
else
