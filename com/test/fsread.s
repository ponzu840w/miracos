...
; -------------------------------------------------------------------
; 新しいリードファンクションのテスト用
; 実装されたら動かなくなるかも 実際動かなくなったが
; 実装後のデバッグ用に再活用するぞ
; -------------------------------------------------------------------
; コマンドライン引数で与えられたテキストファイルをオープンする
; プロンプトで入力されたバイト数を読みだす
; 読み出し結果とともに、FCTRLの内情を吐露する
; FCTRLは、カーネル管轄のファイル状態構造体
; fs/fs.sの267あたりのデバッグコードに依存
; -------------------------------------------------------------------
.INCLUDE "../FXT65.inc"
.INCLUDE "../generic.mac"
...
@INPUT_LENGTH:
  ...
  LDA FD_SAV
  STA ZR1                         ; FDをZR1に
  loadmem16 ZR0,BFPTR             ; バッファポインタをZR0に
  mem2AY16 REQLEN                 ; 要求読み取り長をAYに
...
@SKP_EOF:
  ; ---------------------------------------------------------------
  ;   データ表示
  storeAY16 FCTRL_SAV             ; FCTRLを取得     READがテスト用にAYに開放
  mem2mem16 ACTLEN,ZR2            ; 16bit値を保存 実際の読み取り長 ZR2はREADの作業用
  mem2mem16 SDSEEK,ZR0            ; READがテスト用にZR0に開放
  mem2mem16 BFPTR_NEW,ZR3         ; READ内部でZR3はBFPTR書き込み先アドレス
  ; FCTRL表示
  ; FCTRL_SIZラベル
